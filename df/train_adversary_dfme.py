...
def defense_dcp(bx, teacher, epsilon=0.1, gamma=0.1):
    """
    The official Paper Technique
    ...
    """
    beta = float(epsilon)  # renaming for clarity
    nu = 1000
...
def train(args, teacher, student, generator, device, optimizer, epoch):
    ...
    for i in range(args.epoch_itrs):
        ...
        for _ in range(args.d_iter):
            ...
            with torch.no_grad():
                if args.defense == 'AM_RS_2':
                    t_soft = defense_dcp(fake,teacher,args.epsilon)
        ...
        if i % args.log_interval == 0:
            ...
            example_ct = epoch * args.epoch_itrs + i
            wandb.log({"epoch": epoch, "G_Loss": loss_G.item(), "S_loss": loss_S.item()}, step=example_ct)
...
def main():
    # Training settings
    parser = argparse.ArgumentParser(description='DFME attack')
    ...
    #                     help='eval_data name (default: cifar10)')
    ...
    # NEW
    parser.add_argument('--eval_data', type=str, help='dataset that we eval with; the teacher was trained on this',default='cifar10')
    parser.add_argument('--log_file', type=str, help='path for logging model extraction metrics')
    parser.add_argument('--exp_name', type=str, help='name for exepriment', default='train adversary DFME')
    parser.add_argument('--epsilon', type=str, help='epsilon of perturbations to use for training', default="0.0")
    parser.add_argument('--arch', type=str, help='Use VGG architecture for the model', default='resnet34_8x')
    parser.add_argument('--defense', type=str, help='method used by the teacher for defense', default="None")
    parser.add_argument('--model_filename', type=str, help='model name for saving the student model')
    ...
    run = wandb.init(
        ...
        tags=['adversary', 'DFME', args.eval_data, args.defense],
        name=args.exp_name,
        config=args,
    ...
    args.log_dir = cfg.MODEL_DIR + f"/outputs/{args.log_dir}"
    ...
    global file
    model_dir = cfg.MODEL_DIR + f"/checkpoint/DFME"
    args.model_dir = model_dir
    if (not os.path.exists(model_dir)):
        os.makedirs(model_dir)
    model_path = model_dir + f'/{args.model_filename}'
    ...
    test_loader = DataLoader(eval_data, batch_size=args.batch_size, num_workers=cfg.NUM_WORKERS,
    ...
    myprint("Teacher restored from %s" % (teacher_path))
    print(f"\n\t\tTraining with {args.arch} as a Target\n")
    correct = 0
    with torch.no_grad():
        for i, (data, target) in enumerate(test_loader):
            data, target = data.to(device), target.to(device)
    ...
    print("Calculated number of epochs: ", number_epochs)
    number_epochs = min(args.epochs, number_epochs)
    print("Final number of epochs: ", number_epochs)
    ...
    best_acc = 0
    acc_list = []
    # print("Final number of epochs: ", min(args.epochs, number_epochs))

    for epoch in range(number_epochs): #range(number_epochs): min(args.epochs, number_epochs)
        # Train
        ...
        # Test
        acc, loss = test(args, student=student, generator=generator, device=device, test_loader=test_loader, epoch=epoch)
        acc_list.append(acc)
        wandb.log({'val_loss': loss, 'val_acc': float(100*acc)})
        ...
        if acc > best_acc:
            ...
            os.makedirs(args.log_dir + f"/checkpoint/student_{args.model_id}_{args.eval_data}/", exist_ok=True)
            torch.save(student.state_dict(), args.log_dir + f"/checkpoint/student_{args.model_id}_{args.eval_data}/{args.eval_data}-{name}.pt")
            torch.save(generator.state_dict(), args.log_dir + f"/checkpoint/student_{args.model_id}_{args.eval_data}/{args.eval_data}-{name}-generator.pt")
        # vp.add_scalar('Acc', epoch, acc)
        if args.store_checkpoints:
            torch.save(student.state_dict(), model_path )
            torch.save(generator.state_dict(), model_path.replace('student','generator'))

    myprint("Best Acc=%.6f" % best_acc)
    adversary_acc = 100 * best_acc
    metrics_to_log = \
        {
            "epsilon": args.epsilon,
            "adversary_acc": adversary_acc,
            "adversary_err": 100 - adversary_acc,
        }
    ...
    with open('logs/DFAD-%s.csv' % (args.eval_data), 'a') as f:
        writer = csv.writer(f)
