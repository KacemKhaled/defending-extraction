...
def generate_perturbations(dataset, teacher, student, method, epsilons=[0.1], avg_posteriors=False, sample_surrogates=False,
                           batch_size=64, num_workers=4, device=torch.device("cuda:0"), **kwargs):
    """
    ...
    for (bx, by) in tqdm.tqdm(loader, mininterval=1.0):
        ...
        if avg_posteriors:
            ...
            tmp_outs = []
            for stdt in student:
                out = method(bx, teacher, [stdt.to(device)], epsilons=epsilons, **kwargs).cpu().detach()
                tmp_outs.append(out)
            ...
            out = method(bx.to(device), teacher.to(device), student, epsilons=epsilons,  **kwargs).cpu().detach()
        teacher_pred_perturbed.append(out)
        torch.cuda.synchronize()
        end_time = time.time()
        elapsed_time = end_time - start_time
        len_eps = len(epsilons) if epsilons else 1
        elapsed_time = 1000 * (elapsed_time / len(bx)) / len_eps
    ...
    return teacher_pred_perturbed, list_elapsed_times
...
def method_dcp(bx, teacher, student=None, epsilons=[0.1], gamma=0.1):
    """
...
def method_reverse_sigmoid(bx, teacher, student=None, epsilons=[0.1], gamma=0.1):
    ...
    # for each beta, perturb the posterior with the reverse sigmoid method
    for beta in betas:
        logits = torch.log(torch.clamp(teacher_pred / (1 - teacher_pred), min=1e-12, max=1-1e-12)) # inv_sigmoid
        z = gamma * logits
        r = beta * (torch.sigmoid(z) - 0.5)
        out = teacher_pred - r
        out = out / out.sum(-1).unsqueeze(-1) # Sum to 1 normalization (alpha)
        out = out.unsqueeze(1)  # adding dimension for beta
        all_outs.append(out)
...
def method_gradient_redirection(bx, teacher, students, epsilons=[0.1], backprop_modules=None, override_grad=False):
    """
    original implementation of GRAD defense from : https://github.com/mmazeika/model-stealing-defenses
